...
if __name__ == '__main__':
    ...
    def train_info_batch(epoch):
        ...
        print('epoch:', epoch, '  Training Accuracy:', round(100. * correct /
            total, 3), '  Train loss:', round(train_loss / len(trainloader), 4))
        # Add info batch overhead time 
        print('Info batch overhead time:', info_batch_time)
    ...
    def test(epoch):
        ...
        print('epoch: %d' % epoch, '  Test Acc: %.3f' % cur_acc, 
    ...
    with open(f"./results/results_{args.model}_{args.ratio}_{args.delta}_{args.seed}_{args.use_info_batch}.pkl", 'wb') as f:
