...
if __name__ == '__main__':
    ...
    def train_info_batch(epoch):
        ...
        for batch_idx, blobs in enumerate(trainloader):
            ...
            with torch.cuda.amp.autocast(args.fp16):
                ...
                start_info_batch_time = time.time()
                ...
                loss = trainset.update(loss, args.rescaling)
                info_batch_time += time.time() - start_info_batch_time
        ...
        # Add info batch overhead time 
        safe_print('Info batch overhead time:', info_batch_time)
        train_acc.append(correct / total)
        train_loss_log.append(train_loss / len(trainloader))
        
        return info_batch_time
    ...
    def test(epoch):
        ...
        valid_loss_log.append(cur_loss)
        

    total_time = 0
    total_info_batch_time = 0
    ...
    for epoch in range(args.num_epoch):
        ...
        if args.use_info_batch:
            epoch_info_batch_time = train_info_batch(epoch)
            total_info_batch_time += epoch_info_batch_time
        else:
            train_normal(epoch)
    ...
    if args.use_info_batch:
        ...
        prune_percentage = trainloader.sampler.get_prune_percentage()
        safe_print('Prune percentage:', prune_percentage)
        safe_print('Total info batch overhead time: ', total_info_batch_time)
